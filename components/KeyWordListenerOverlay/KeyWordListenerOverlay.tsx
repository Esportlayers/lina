...
const variants: Variants = {
  hidden: { width: "0%", transition: { duration: 0.24 } },
  visible: { width: "auto", transition: { duration: 0.24 } },
};

function mergeQuestions(
  oldState: KeywordMessage["value"][],
  newState: KeywordMessage["value"][]
...
export default function KeyWordListenerOverlay(): ReactElement {
  ...
  const { value } = useTetherMessageListener<KeywordMessage>(
  ...
  const onShowOverlay = useCallback(
    async (data) => {
      ...
      if (!activeOverlay) {
        await post(
          process.env.API_URL + "/live/keywordQuestion",
          { data },
          getDefaultHeader()
        );
      }
  ...
  return (
    <AnimatePresence>
      {user?.useKeywordListener && (
        <motion.div
          initial={"hidden"}
          animate={"visible"}
          exit={"hidden"}
          variants={variants}
          className={"keywordListenerOverlay"}
        >
          <div
            className={"keywordOverlay"}
          ...
          >
            ...
            <div className={"gridContainer"}>
              <div className={"grid"}>
                {questions.map(({ name, message, logo, time }) => (
